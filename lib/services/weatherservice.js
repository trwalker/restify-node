...
module.exports = function(jsonClientService) { 
  return (function(jsonClientService) {
  	...
  	function getWeatherData(postalCode, success, error) {
  		if(!isInputValid(postalCode, success, error)) {
  			weatherServiceError(error);
			}
			else {
	  		var cachedWeatherData = weather.postalCodeCachedData[postalCode];

				if(cachedWeatherData) {
					success(cachedWeatherData);
				}
				else {
					var path = weather.pathFormat.replace('{0}', postalCode);
					
					jsonClientService.get(weather.url, 
																path,
																null,	
																function(res, obj) { weatherServiceSuccess(postalCode, obj, success, error); }, 
																function(res, err) { weatherServiceError(error); });
				}
  		}
		}

		function isInputValid(postalCode, success, error) {
			var valid = false;
